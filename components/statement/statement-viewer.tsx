...
export function StatementViewer({ cardStatements }: StatementViewerProps) {
  ...
  const [selectableExpenses, setSelectableExpenses] = useState<
    SelectableExpense[]
  >([]);
  ...
  const { selectedExpenses, totalExpenses, expenseCount, averageExpense } =
    useMemo(() => {
      const selected = selectableExpenses.filter(
            ...
            <div className="flex flex-wrap gap-2">
              {cardStatements.map((card) => (
                <Button
                  key={card.cardName}
                  variant={
                    selectedCards.includes(card.cardName)
                      ? "default"
                      : "outline"
                  }
                  size="sm"
            ...
            <div className="flex flex-wrap gap-2">
              {availableMonths.map((month) => (
                <Button
                  key={month}
                  variant={
                    selectedMonths.includes(month) ? "default" : "outline"
                  }
                  size="sm"
          ...
          <>
            <div className="flex flex-col sm:flex-row justify-between items-center mb-4">
              <div className="text-sm text-gray-500 mb-2 sm:mb-0">
                Showing {selectedExpenses.length} of {selectableExpenses.length}{" "}
                transactions
              </div>
                    ...
                    <TableBody>
                      {selectableExpenses.map((expense) => (
                        ...
                        >
                          ...
                          <TableCell>
                            {formatCardName(expense.cardName)}
                          </TableCell>
                    ...
                    <Card key={statement.id}>
                      <CardHeader className="p-4 pb-2">
                        <CardTitle className="text-lg">
                          {formatCardName(statement.cardName)} -{" "}
                          {statement.month}
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="p-4 pt-0">
